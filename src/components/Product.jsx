          ...
          <ul className='flex justify-center gap-2 text-left'>
            {allCategories.map((cat, index) => (
              <li key={index} className='group'>
                <Link href={`/featured/${cat}`} className='text-sm whitespace-nowrap group-hover:underline underline-offset-2'>
                  {cat.replace(/_/g, ' ')}
                </Link>
              </li>
            ))}
      ...
      <div className='flex'>
        <div className='hidden md:flex justify-center md:w-2/12'>
          <ul className='flex flex-col justify-center gap-4 text-left h-96 sticky top-8 border-r px-16'>
            {allCategories.map((cat, index) => (
              <li key={index} className='group'>
                <Link href={`/featured/${cat}`} className='group-hover:underline underline-offset-2'>
                  {cat.replace(/_/g, ' ').toUpperCase()}
                </Link>
              </li>
            ))}
            <li className='hover:underline underline-offset-2'><Link href='/featured/shop_all'>Shop All</Link></li>
          </ul>
        </div>
