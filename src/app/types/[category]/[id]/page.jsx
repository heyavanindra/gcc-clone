...
const getProductData = async (category, id) => {
  try {
    let res = await fetch(`${NEXT_PUBLIC_HOST_URL}/api/types/${category}/${id}`,{cache: 'no-store'});
    res = await res.json();
    return res;
  } catch (err) {
    console.log(err);
      ...
      {item.length > 0 ? (
        ...
        <div>
          ...
          <div className='py-4'>
            ...
            <SizeSelector id={item[0]._id} sizes={item[0].quantity.size} amount={item[0].amount} title={item[0].title} img={item[0].img} category={category} productType={'types'} 
